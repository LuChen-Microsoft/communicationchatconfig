...
import { createDiagnosticCollector, getDiscriminator, getEncode, getFormat, getKnownValues, getNamespaceFullName, getVisibility, ignoreDiagnostics, isErrorModel, isNeverType, } from "@typespec/compiler";
import { Visibility, getAuthentication, getServers, isHeader, isStatusCode, } from "@typespec/http";
import { getAccessOverride, getUsageOverride, isExclude, isInclude, isInternal, listClients, listOperationGroups, listOperationsInOperationGroup, shouldFlattenProperty, shouldGenerateConvenient, } from "./decorators.js";
import { UsageFlags, getKnownScalars, isSdkBuiltInKind, } from "./interfaces.js";
import { createGeneratedName, getAnyType, getAvailableApiVersions, getDocHelper, getLocationOfOperation, getNonNullOptions, getNullOption, getSdkTypeBaseHelper, intOrFloat, isAzureCoreModel, isMultipartFormData, isMultipartOperation, isNeverOrVoidType, updateWithApiVersionInformation, } from "./internal-utils.js";
...
function getEncodeHelper(context, type, kind) {
 ...
 */
function getSdkBuiltInTypeWithDiagnostics(context, type) {
    const diagnostics = createDiagnosticCollector();
...
export function getSdkTupleWithDiagnostics(context, type, operation) {
    ...
    return diagnostics.wrap({
        ...
        values: type.values.map((x) => diagnostics.pipe(getClientTypeWithDiagnostics(context, x, operation))),
    });
}
...
export function getSdkUnionWithDiagnostics(context, type, operation) {
    ...
    if (retval === undefined) {
        retval = {
            ...
            values: nonNullOptions.map((x) => diagnostics.pipe(getClientTypeWithDiagnostics(context, x, operation))),
        };
    }
...
function getUnionAsEnumValueType(context, union) {
    ...
    for (const option of nonNullOptions) {
        if (option.kind === "Union") {
            ...
            return getClientType(context, option);
        }
    }
    return undefined;
}
...
export function getSdkUnionEnum(context, type, operation) {
    ...
    if (!sdkType) {
        ...
        sdkType = {
            ...
            description: docWrapper.description,
            details: docWrapper.details,
            valueType: getUnionAsEnumValueType(context, type.union) ??
    ...
    switch (type.kind) {
        ...
        case "Scalar":
            if (!context.program.checker.isStdType(type) && type.kind === "Scalar" && type.baseScalar) {
                const baseType = diagnostics.pipe(getClientTypeWithDiagnostics(context, type.baseScalar, operation));
...
export function getSdkModelPropertyType(context, type, operation) {
    ...
    if (operation) {
        ...
        operationIsMultipart = Boolean(httpOperation && httpOperation.parameters.body?.contentTypes.includes("multipart/form-data"));
    }
...
function updateModelsMap(context, type, sdkType, operation) {
    ...
    if (context.operationModelsMap === undefined) {
        context.operationModelsMap = new Map();
    }
        ...
        if (sdkType.kind === "model") {
            for (const prop of sdkType.properties) {
                if (prop.type.kind === "model" || prop.type.kind === "enum") {
                    updateModelsMap(context, prop.type.__raw, prop.type, operation);
                }
                if (prop.type.kind === "array" || prop.type.kind === "dict") {
                    updateModelsMap(context, prop.type.valueType.__raw, prop.type.valueType, operation);
                }
                if (prop.type.kind === "union") {
                    for (const unionType of prop.type.values) {
                        updateModelsMap(context, unionType.__raw, unionType, operation);
                    }
                }
            }
            if (sdkType.baseModel) {
                updateModelsMap(context, sdkType.baseModel.__raw, sdkType.baseModel, operation);
            }
            if (sdkType.additionalProperties) {
                updateModelsMap(context, sdkType.additionalProperties.__raw, sdkType.additionalProperties, operation);
            }
            if (sdkType.discriminatedSubtypes) {
                for (const subtype of Object.values(sdkType.discriminatedSubtypes)) {
                    updateModelsMap(context, subtype.__raw, subtype, operation);
                }
            }
        }
    }
}
function updateUsageOfModel(context, usage, type, options) {
    options = options ?? {};
    ...
    const usageOverride = getUsageOverride(context, type.__raw);
    if (usageOverride) {
...
function updateTypesFromOperation(context, operation) {
    ...
    if (httpBody && !isNeverOrVoidType(httpBody.type)) {
        const sdkType = diagnostics.pipe(getClientTypeWithDiagnostics(context, httpBody.type, operation));
        if (generateConvenient) {
    ...
    for (const response of httpOperation.responses) {
        for (const innerResponse of response.responses) {
            if (innerResponse.body?.type && !isNeverOrVoidType(innerResponse.body.type)) {
                const sdkType = diagnostics.pipe(getClientTypeWithDiagnostics(context, innerResponse.body.type, operation));
                if (generateConvenient) {
...
function updateAccessOfModel(context) {
    for (const [type, sdkType] of context.modelsMap?.entries() ?? []) {
        const internal = isInternal(context, type); // eslint-disable-line deprecation/deprecation
        if (internal) {
...
function verifyNoConflictingMultipartModelUsage(context) {
    const diagnostics = createDiagnosticCollector();
    for (const [operation, modelMap] of context.operationModelsMap) {
...
function modelChecks(context) {
    return verifyNoConflictingMultipartModelUsage(context);
}
function filterOutModels(context) {
    for (const [type, sdkType] of context.modelsMap?.entries() ?? []) {
        if (type.kind === "Model") {
            if (isExclude(context, type))
                sdkType.usage = UsageFlags.None; // eslint-disable-line deprecation/deprecation
        }
