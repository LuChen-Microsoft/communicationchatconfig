...
import { getAvailableApiVersions, getDocHelper, getLocationOfOperation, isAcceptHeader, isContentTypeHeader, isNeverOrVoidType, isSubscriptionId, } from "./internal-utils.js";
import { createDiagnostic } from "./lib.js";
import { getCrossLanguageDefinitionId, getEffectivePayloadType } from "./public-utils.js";
import { addEncodeInfo, addFormatInfo, getClientTypeWithDiagnostics, getSdkModelPropertyTypeBase, } from "./types.js";
...
function getSdkHttpParameters(context, httpOperation, methodParameters, responseBody) {
    ...
    if (tspBody && tspBody?.bodyKind !== "multipart") {
        ...
        if (tspBody.parameter && !isNeverOrVoidType(tspBody.parameter.type)) {
            const getParamResponse = diagnostics.pipe(getSdkHttpParameter(context, tspBody.parameter, httpOperation.operation, "body"));
...
export function getCorrespondingMethodParams(context, operation, methodParameters, serviceParam) {
    ...
    if (serviceParam.kind === "body" && serviceParamType.kind === "model") {
        ...
        if (directBodyProperty)
            return diagnostics.wrap([directBodyProperty]);
        let correspondingProperties = methodParameters.filter((x) => serviceParamPropertyNames.includes(x.name));
        for (const methodParam of methodParameters) {
            const methodParamIterable = methodParam.type.kind === "model" ? methodParam.type.properties : [methodParam];
            correspondingProperties = correspondingProperties.concat(methodParamIterable.filter((x) => serviceParamPropertyNames.includes(x.name) &&
