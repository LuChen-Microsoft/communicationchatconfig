import { createDiagnosticCollector, getEffectiveModelType, getFriendlyName, getNamespaceFullName, getProjectedName, ignoreDiagnostics, isErrorModel, listServices, resolveEncodedName, } from "@typespec/compiler";
import { getHeaderFieldName, getHttpOperation, getPathParamName, getQueryParamName, isMetadata, isStatusCode, } from "@typespec/http";
 ...
 * @returns
 */
export function getCrossLanguageDefinitionId(context, type, appendNamespace = true) {
 ...
 */
export function isErrorOrChildOfError(context, model) {
    const errorDecorator = isErrorModel(context.program, model);
    if (errorDecorator)
        return true;
    let baseModel = model.baseModel;
    while (baseModel) {
        if (isErrorModel(context.program, baseModel))
            return true;
